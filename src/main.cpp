...
int main() {
    ...
    glEnable(GL_DEPTH_TEST); glEnable(GL_CULL_FACE); glEnable(GL_LIGHTING); glEnable(GL_LIGHT0); glEnable(GL_NORMALIZE); glShadeModel(GL_SMOOTH);
    ...
    const GLfloat fogColor[] = {0.025F, 0.07F, 0.08F, 1.0F}; glEnable(GL_FOG); glFogi(GL_FOG_MODE, GL_EXP2); glFogfv(GL_FOG_COLOR, fogColor); glFogf(GL_FOG_DENSITY, 0.018F);
    ...
    while (running) {
        ...
        glTranslatef(0.0F, -3.8F, -28.0F); glRotatef(18.0F, 1, 0, 0); glRotatef(yaw, 0, 1, 0); glTranslatef(-heroX, 0.0F, -heroZ); glRotatef(-90.0F, 1, 0, 0);
        environment.draw();
